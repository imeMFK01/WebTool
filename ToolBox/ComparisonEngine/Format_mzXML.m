%Reading mascot file
function Format_mzXML(MascotFile,OutputDir)
...
ProjectData = uigetdir(pwd,'Select mzXML Data Folder' );
...
for number=1:size(Replicates,1)
    ...
    if(strcmp (CurrentReplicate, '.') || isempty(CurrentReplicate) || strcmp (CurrentReplicate, '..'))
        ...
        %  Relicates
        dir_sub= char(strcat(ProjectData,strcat('\',Replicates(number).name)));
